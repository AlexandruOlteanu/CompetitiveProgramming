...
*/
// Link: https://codeforces.com/problemset/problem/2109/D
#include <bits/stdc++.h>
...
void CoreCompute(const int testIdx, const bool isLastTest) {
    ...
    for (int i = 1; i <= l; ++i) {
        cin >> s[i];
        if (s[i] % 2) imp.push_back(s[i]);
        else par.push_back(s[i]);
    }
    ...
    // dbg(testIdx, dist);
    int sum = accumulate(all(par), 0);
    sum = accumulate(all(imp), sum);

    auto check = [=](int d) -> bool {
        if (sum < d) return false;
        if (d % 2 == 0) {
            if (imp.size() % 2 == 0) return true;
            int cur = sum - imp[0];
